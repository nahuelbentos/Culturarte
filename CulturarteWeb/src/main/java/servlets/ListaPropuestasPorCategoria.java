	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (DtPropuesta prop : propuestas) {
				...
				if ("ingresada".equals(prop.getEstadoActual().getValue())) {
					listaIngresada.add(dtPM);
				} else if ("publicada".equals(prop.getEstadoActual().getValue())) {
					listaPublicada.add(dtPM);
				} else if ("enFinanciacion".equals(prop.getEstadoActual().getValue())) {
					listaEnFinanciacion.add(dtPM);
				} else if ("financiada".equals(prop.getEstadoActual().getValue())) {
					listaFinanciada.add(dtPM);
				} else if ("noFinanciada".equals(prop.getEstadoActual().getValue())) {
					listaNoFinanciada.add(dtPM);
				} else if ("cancelada".equals(prop.getEstadoActual().getValue())) {
